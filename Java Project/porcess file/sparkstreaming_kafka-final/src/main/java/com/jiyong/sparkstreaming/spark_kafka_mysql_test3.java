...
public class spark_kafka_mysql_test3 {
                        ...
                        if(hasNextBase){
                            ...
                            if(!hasNext){
                                if(flag == 0){
                                    ...
                                    String whethernew = new String("no");
                                    String insertsql = "INSERT INTO shop_info(si_company_ID,si_shop_ID,memberid,Longitude,Latitude,obtainted,recommend,saturation,whethernew) VALUES('" + si_company_ID + "','" + si_shop_ID + "','" + memberid + "','" + Longitude + "','" + Latitude + "',1,0,0,,'" + whethernew + "')";
                                    stmtShop.executeUpdate(insertsql);
                                }
                            }
                            if(hasNext) {
                                if(flag == 0){
                                    ...
                                    int recommend = resultSetshop.getInt("recommend");
                                    double saturation = (double) obtainted/(recommend);
                                    String updatesql = "UPDATE shop_info SET Longitude = '"+Longitude+"',Latitude = '"+Latitude+"',obtainted = '"+obtainted+"',saturation = '"+saturation+"',whethernew = '"+whethernew+"' WHERE si_shop_ID = '"+si_shop_ID+"'";
                                    stmtShop.executeUpdate(updatesql);
                                }
                            }
                        }
